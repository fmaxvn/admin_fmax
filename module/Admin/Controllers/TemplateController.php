<?php

namespace module\Admin\Controllers;

use Core\Database\DBHandler; // ✅ Load module database
use Core\Helper\Validator;
use Core\Helper\ViewHelper;

class TemplateController extends ViewHelper
{

    private $table = " jp_template";
    private $tableCategory = " jp_template_category";

    public function index()
    {
        $db = new DBHandler($this->table);
        $data = array();

        $params = $this->getParams();

        // ✅ **Xây dựng điều kiện truy vấn**
        $conditions = [];

        $options = [
            'order_by' => ["id desc"],
            'columns' => 'jp_template.*, jp_template_category.name AS type_name',
            'joins' => [
                ['LEFT JOIN', 'jp_template_category', 'jp_template.type = jp_template_category.id'], // JOIN bảng jp_domain
            ]
        ];
        // ✅ **Truy vấn danh sách domain**
        $data["listTemplate"] = $db->getList($conditions, $options);
        foreach ($data["listTemplate"] as $key => $val) {
            if (!empty($val['images'])) {
                $data["listTemplate"][$key]['images'] = $this->getImageUrl($val['images']);
            } else {
                $data["listTemplate"][$key]['images'] = URL_ASSETS . '/images/default.jpg';
            }
        }
        return $this->getLayout($data);
    }

    /**
     * Hàm thêm mới
     * @return string
     */
    public function add()
    {
        $view = new ViewHelper();

        $dbCategory = new DBHandler($this->tableCategory);
        $data = [];
        $data['listTypeTemplate'] = $dbCategory->getList();

        // Xử lý form submit
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $db = new DBHandler($this->table);
            // Validate dữ liệu
            $validator = new Validator();
            $validator->validate($_POST, [
                'name' => ['required'],
                'name_db_template' => ['required'],
                'type' => ['required', 'in:1,2,3,4'],
                'url_page_home_template' => ['required'],
            ]);

            if ($validator->fails()) {
                $data['errors'] = $validator->errors();
                $data['old'] = $_POST; // Giữ lại dữ liệu đã nhập
                return $view->getLayout($data);
            }

            // Insert trực tiếp dữ liệu POST vào database
            if ($db->insert($_POST)) {
                $data['success'] = "Thêm mới domain thành công.";
            } else {
                $data['error'] = "Có lỗi xảy ra khi thêm mới. Vui lòng thử lại.";
                $data['old'] = $_POST; // Giữ lại dữ liệu đã nhập
            }
        }

        return $view->getLayout($data);
    }

    /**
     * hàm chỉnh sửa
     * @return string
     */
    public function edit()
    {
        $db = new DBHandler($this->table);
        $dbCategory = new DBHandler($this->tableCategory);

        $view = new ViewHelper();
        $params = $this->getParams();
        $objId = $params['id'];
        // ✅ **Truy vấn danh sách thành viên với các cột cần thiết**
        $data['templateDetail'] = $templateDetail = $db->getOne(['id' => $objId]);
        $data['listTypeTemplate'] = $dbCategory->getList();
        if (!$templateDetail) {
            return $view->getLayout(['error' => "Đơn hàng không tồn tại"]);
        }
        // ✅ Nếu form được submit (POST request)
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $validator = new Validator();
            $validator->validate($_POST, [
                'name' => ['required'],
                'name_db_template' => ['required'],
                'type' => ['required', 'in:1,2,3,4'],
                'url_page_home_template' => ['required'],
            ]);

            // Kiểm tra nếu có lỗi
            if ($validator->fails()) {
                $data['errors'] = $validator->errors(); // ✅ Truyền danh sách lỗi ra view
                $data['old'] = $_POST; // ✅ Giữ lại dữ liệu đã nhập
                return $view->getLayout(array_merge(['templateDetail' => $templateDetail], $data));
            }

            // Lấy toàn bộ dữ liệu từ form
            $fields = [
                'name',
                'name_db_template',
                'type',
                'url_page_home_template',
                'images',
                'showview',
            ];

            $updateData = [];

            foreach ($fields as $field) {
                if (isset($_POST[$field])) {
                    $updateData[$field] = $_POST[$field];
                }
            }

            // ✅ Cập nhật dữ liệu vào database
            if (!empty($updateData)) {
                $success = $db->update($updateData, ['id' => $objId]);
                if ($success) {
                    $data['templateDetail'] = array_merge($templateDetail, $updateData); // Cập nhật dữ liệu mới 
                    $data['success'] = "Cập nhật thành công";
                    header("Location: /admin/template/index");
                    exit();
                } else {
                    $data['error'] = "Không có dữ liệu nào được cập nhật";
                }
                return $view->getLayout($data);
            }
        }
        // ✅ Nếu chỉ là GET request, hiển thị form chỉnh sửa
        return $view->getLayout($data);
    }

    public function category()
    {
        $db = new DBHandler($this->tableCategory);

        // ✅ **Lấy tham số từ URL**
        $search = $_GET['search'] ?? ''; // Tìm kiếm theo tên
        $filter = $_GET['filter'] ?? ''; // Lọc theo loại domain (1 = VN, 2 = QT)
        $sort = $_GET['sort'] ?? 'desc'; // Sắp xếp (asc/desc)
        $data = array();

        $params = $this->getParams();

        // ✅ **Xây dựng điều kiện truy vấn**
        $conditions = [];
        if (!empty($search)) {
            $conditions['name'] = ['LIKE', "%$search%"];
        }
        if (!empty($filter)) {
            $conditions['type'] = $filter; // '1' = VN, '2' = QT
        }

        $options = [
            'order_by' => ["id asc"],
            'columns' => 'id, name, images, description',
        ];
        // ✅ **Truy vấn danh sách**
        $data["listCategory"] = $db->getList($conditions, $options);
        foreach ($data["listCategory"] as $key => $val) {
            if (!empty($val['images'])) {
                $data["listCategory"][$key]['images'] = $this->getImageUrl($val['images']);
            } else {
                $data["listCategory"][$key]['images'] = URL_ASSETS . '/images/default.jpg';
            }
        }

        return $this->getLayout($data);
    }

    public function category_add()
    {
        $view = new ViewHelper();
        $data = [];

        // Xử lý form submit
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $db = new DBHandler($this->tableCategory);

            // Validate dữ liệu
            $validator = new Validator();
            $validator->validate($_POST, [
                'name' => ['required'],
                'description' => ['required'],
            ]);

            if ($validator->fails()) {
                $data['errors'] = $validator->errors();
                $data['old'] = $_POST; // Giữ lại dữ liệu đã nhập
                return $view->getLayout($data);
            }

            // Insert trực tiếp dữ liệu POST vào database
            if ($db->insert($_POST)) {
                $data['success'] = "Thêm mới thành công.";
            } else {
                $data['error'] = "Có lỗi xảy ra khi thêm mới. Vui lòng thử lại.";
                $data['old'] = $_POST; // Giữ lại dữ liệu đã nhập
            }
        }

        return $view->getLayout($data);
    }

    /**
     * hàm chỉnh sửa
     * @return string
     */
    public function category_edit()
    {
        $db = new DBHandler($this->tableCategory);

        $view = new ViewHelper();
        $params = $this->getParams();
        $objId = $params['id'];
        // ✅ **Truy vấn danh sách thành viên với các cột cần thiết**
        $objDetail = $db->getOne(['id' => $objId]);

        if (!$objDetail) {
            return $view->getLayout(['error' => "Dữ liệu không tồn tại"]);
        }
        // ✅ Nếu form được submit (POST request)
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $validator = new Validator();
            $validator->validate($_POST, [
                'name' => ['required'],
                'description' => ['required'],
            ]);

            // Kiểm tra nếu có lỗi
            if ($validator->fails()) {
                $data['errors'] = $validator->errors(); // ✅ Truyền danh sách lỗi ra view
                $data['old'] = $_POST; // ✅ Giữ lại dữ liệu đã nhập
                return $view->getLayout(array_merge(['objDetail' => $objDetail], $data));
            }

            // Lấy toàn bộ dữ liệu từ form
            $fields = [
                'name',
                'description',
            ];

            $updateData = [];
            foreach ($fields as $field) {
                if (isset($_POST[$field])) {
                    $updateData[$field] = $_POST[$field];
                }
            }

            // ✅ Cập nhật dữ liệu vào database
            if (!empty($updateData)) {
                $success = $db->update($updateData, ['id' => $objId]);
                if ($success) {
                    $data['objDetail'] = array_merge($objDetail, $updateData); // ✅ Cập nhật lại dữ liệu mới
                    $data['success'] = "Cập nhật thành công";
                    header("Location: /admin/template/category");
                    exit();
                } else {
                    $data['error'] = "Không có dữ liệu nào được cập nhật";
                }
                return $view->getLayout($data);
            }
        }
        $data['objDetail'] = $objDetail;

        // ✅ Trả về View với đầy đủ `$data`
        return $view->getLayout($data);
    }


    /**
     * hàm cập nhật thứ tự ưu tiên
     * @return never
     */
    public function togglePriority()
    {
        // Kết nối Database
        $db = new DBHandler($this->table);

        // Kiểm tra phương thức POST
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            http_response_code(405); // Method Not Allowed
            echo json_encode(['success' => false, 'message' => 'Phương thức không hợp lệ']);
            exit();
        }

        // Lấy dữ liệu JSON từ request
        $input = json_decode(file_get_contents("php://input"), true);

        // Kiểm tra dữ liệu hợp lệ
        if (!isset($input['id']) || !isset($input['priority'])) {
            http_response_code(400); // Bad Request
            echo json_encode(['success' => false, 'message' => 'Dữ liệu không hợp lệ']);
            exit();
        }

        $domainId = (int) $input['id'];
        $newPriority = (int) $input['priority'];

        // Kiểm tra domain tồn tại
        $domain = $db->getOne(['id' => $domainId]);

        if (!$domain) {
            http_response_code(404); // Not Found
            echo json_encode(['success' => false, 'message' => 'Domain không tồn tại']);
            exit();
        }

        // Cập nhật trạng thái `priority`
        $updatePriority = $db->update(['priority' => $newPriority], ['id' => $domainId]);

        if ($updatePriority) {
            echo json_encode(['success' => true, 'message' => 'Cập nhật ưu tiên thành công']);
        } else {
            http_response_code(500); // Internal Server Error
            echo json_encode(['success' => false, 'message' => 'Có lỗi xảy ra khi cập nhật']);
        }

        exit();
    }

    /**
     * Hàm Xóa
     */
    public function delete()
    {
        // Chỉ chấp nhận POST request để tránh CSRF
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('HTTP/1.1 403 Forbidden');
            echo json_encode(['status' => 'error', 'message' => 'Method not allowed']);
            exit;
        }
        $objId = $_POST['id'] ?? '';
        $db = new DBHandler($this->table);

        // Lấy thông tin user để xóa avatar nếu có
        $objDetail = $db->getOne(['id' => $objId]);

        if ($objDetail && $db->delete(['id' => $objId])) {
            echo json_encode(['status' => 'success', 'message' => 'Xóa thành công.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Có lỗi xảy ra khi xóa.']);
        }
        exit;
    }
    /**
     * Hàm Xóa
     */
    public function deleteCategory()
    {
        // Chỉ chấp nhận POST request để tránh CSRF
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            header('HTTP/1.1 403 Forbidden');
            echo json_encode(['status' => 'error', 'message' => 'Method not allowed']);
            exit;
        }
        $objId = $_POST['id'] ?? '';
        $db = new DBHandler($this->tableCategory);

        // Lấy thông tin user để xóa avatar nếu có
        $objDetail = $db->getOne(['id' => $objId]);

        if ($objDetail && $db->delete(['id' => $objId])) {
            echo json_encode(['status' => 'success', 'message' => 'Xóa thành công.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Có lỗi xảy ra khi xóa.']);
        }
        exit;
    }
}
